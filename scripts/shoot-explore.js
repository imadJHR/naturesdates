...
(async () => {
  ...
  // normal state
  await p.screenshot({ path:"verify-shots/explore-normal.png", clip:{x:0,y:Math.max(0,box.top-20),width:1280,height:Math.min(box.h+40,520)} });
  // hover second card
  const links = await p.$$('.category-explore a');
  if(links[1]){ await links[1].hover(); await new Promise(r=>setTimeout(r,400)); }
  await p.screenshot({ path:"verify-shots/explore-hover.png", clip:{x:0,y:Math.max(0,box.top-20),width:1280,height:Math.min(box.h+40,520)} });
  console.log("DONE", links.length, "cards");
  await b.close();
